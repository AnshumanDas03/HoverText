const _C = document.getElementById("c"),
    CT = _C.getContext("2d"),
    DIR = ["x", "y"],
    TEXTBOX = { str: "" }, // Initialize with an empty string
    PALETTE = [
        "ffad70",
        "f7d297",
        "edb9a1",
...
function write() {
    ...
    TEXTBOX.x = 0.5 * (_C.width - TEXTBOX.w);
    TEXTBOX.y = 0.5 * (_C.height - TEXTBOX.h);

    let grd = CT.createLinearGradient(
        TEXTBOX.x,
        TEXTBOX.y,
        TEXTBOX.x + TEXTBOX.w,
        TEXTBOX.y + TEXTBOX.h
    ...
    CT.fillText(TEXTBOX.str, 0.5 * _C.width, 0.5 * _C.height);
...
function size() {
    ...
    ["width", "height"].forEach(
        (c) => (_C[c] = +getComputedStyle(_C)[c].slice(0, -2))
    );
...
// Event listener for the "Apply Effect" button
const applyEffectButton = document.getElementById("applyEffect");
applyEffectButton.addEventListener("click", () => {
    const inputTextElement = document.getElementById("textInput");
    TEXTBOX.str = inputTextElement.value; // Update the TEXTBOX with user input
    size(); // Trigger the effect with the updated text
});
